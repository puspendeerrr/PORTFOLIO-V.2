...
/**
 * Get all documentations - GET /api/documentations
 * @returns {Promise<{success: boolean, data: Array}>}
 */
export const getDocumentations = async () => {
  try {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.message || 'Failed to fetch documentations');
    }
...
/**
 * Create documentation - POST /api/documentations
 * @param {string} token - JWT token
 * @param {FormData} docData - Documentation form data (title, date, points, file)
 * @returns {Promise<{success: boolean, data: Object}>}
 */
export const createDocumentation = async (token, docData) => {
  try {
    const response = await fetch(`${BASE_URL}/documentations`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      const error = await response.json();
      throw new Error(error.message || 'Failed to create documentation');
    }
...
/**
 * Delete documentation - DELETE /api/documentations/:id
 * @param {string} token - JWT token
 * @param {string} docId - Documentation ID
 * @returns {Promise<{success: boolean, data: Object}>}
 */
export const deleteDocumentation = async (token, docId) => {
  try {
    const response = await fetch(`${BASE_URL}/documentations/${docId}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
