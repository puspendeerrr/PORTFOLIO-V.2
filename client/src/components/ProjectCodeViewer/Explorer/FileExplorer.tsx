...
const FileExplorerNode: React.FC<FileExplorerNodeProps> = ({
  ...
  return (
    <div className="file-explorer-node">
      ...
      >
        {isFolder && (
          <span className={`file-explorer-icon file-explorer-icon--folder ${isExpanded ? "file-explorer-icon--expanded" : ""}`} style={{ display: 'flex', alignItems: 'center' }}>
            {isExpanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
          </span>
        )}
        {!isFolder && <span className="file-explorer-icon file-explorer-icon--file" style={{ display: 'flex', alignItems: 'center' }}><FileCode size={14} /></span>}
