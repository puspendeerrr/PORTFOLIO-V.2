...
export const FileNode: React.FC<FileNodeProps> = ({
  ...
  return (
    <div className="file-node">
      ...
      >
        {isFolder && (
          <span className={`file-node-icon file-node-icon--folder ${isExpanded ? "file-node-icon--expanded" : ""}`} style={{ display: 'flex', alignItems: 'center' }}>
            {isExpanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
          </span>
        )}
        {!isFolder && <span className="file-node-icon file-node-icon--file" style={{ display: 'flex', alignItems: 'center' }}><FileCode size={14} /></span>}
