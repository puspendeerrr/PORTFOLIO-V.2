...
import { Check, X, FolderUp } from 'lucide-react';
...
const UploadFolderButton: React.FC<UploadFolderButtonProps> = ({ token, onUploadSuccess }) => {
  ...
  return (
    <div className="upload-folder-wrapper">
      ...
      >
        <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><FolderUp size={16} /> Upload Folder</span>
      </button>
      ...
      {uploadStatus === 'success' && (
        <div className="upload-status-message success" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          <Check size={14} /> {statusMessage}
        </div>
      )}

      {uploadStatus === 'error' && (
        <div className="upload-status-message error" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          <X size={14} /> {statusMessage}
        </div>
      )}
    </div>
  );
