    ...
    <div className="upload-project-files">
      ...
      >
        <button
          ...
          type="button"
        >
          <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><FileCode size={16} /> Upload Code File</span>
        </button>
      ...
      {selectedFile && (
        <div className="file-selected-info">
          <span className="file-name" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><Check size={14} /> {selectedFile.name}</span>
          <span className="file-size">({formatFileSize(selectedFile.size)})</span>
        </div>
      )}

      {error && (
          <div className="upload-error" style={{ display: 'flex', alignItems: 'center', gap: '4px', color: 'var(--color-danger)', fontSize: '0.9rem' }}>
            <X size={14} /> {error}
          </div>
      )}
    </div>
