 ...
 */
const CreateFileForm = ({ token, onFileCreated }: { token: string; onFileCreated?: () => void }) => {
  ...
  if (!showForm) {
    return (
      ...
      >
        <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><Plus size={16} /> Create New File</span>
      </button>
    );
  }
        ...
        <form onSubmit={handleSubmit} className="create-file-form">
          ...
          {error && (
            <div className="error-message">
              <span className="error-icon" style={{ display: 'flex', alignItems: 'center' }}><AlertTriangle size={16} /></span>
              {error}
            </div>
          )}
          ...
          {success && (
            <div className="success-message">
              <span className="success-icon" style={{ display: 'flex', alignItems: 'center' }}><Check size={16} /></span>
              {success}
            </div>
          )}
